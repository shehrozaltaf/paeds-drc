...
class Form extends CI_Controller
{
    ...
    public function aims_applications_edit($slug)
    {
        $getData = array();
        $MForm_View = new MForm_View();
        $getData['getData'] = $MForm_View->getFormEditDetail($slug);
    ...
    public function aims_draft_edit($slug)
    {
        $getData = array();
        $MForm_View = new MForm_View();
        $getData['getData'] = $MForm_View->getFormDraftDetail($slug);
    ...
    public function sendEmail()
    {
        ...
        if (isset($_POST['isEdit']) && $_POST['isEdit'] == 'Yes') {
            ...
            if (isset($_POST['isEdit']) && $_POST['isEdit'] == 'submit_new' && $_POST['isEdit'] != 'No') {
                $idForm = $_POST['idApplication'];
                $insertArr['assignTo'] = $_POST['idApplication'];
            } else {
                $maxIdForm = $MForm_View->getMaxIdForm();
                $idForm = $maxIdForm[0]->maxIdForm + 1;
                $getMinimumReviewer = $MForm_View->getMinimumReviewer();
                $assignTo = $getMinimumReviewer[0]->idUser;
                $insertArr['assignTo'] = $assignTo;
            }
            $insertArr['idApplication'] = $idForm;
            $insertArr['createdBy'] = $createdby;
            $insertArr['createdDateTime'] = date('Y-m-d H:i:s');
            if (empty($_FILES['attachment']['tmp_name'][0]) || $_FILES['attachment']['size'][0] == 0 || $_FILES['attachment']['name'][0] == "") {
                $file_name = '';
            } else {
...
' . $Content . '
    ...
    public function updateCommentFile()
    {
        $Custom = new Custom();
        if (isset($_POST['idApplicationForm']) && $_POST['idApplicationForm'] != '') {
            $idForm = $_POST['idApplicationForm'];
            $ReviewNo = $_POST['ReviewNo'];
            $idApplicationComment = $_POST['idApplicationComment'];
            ...
            $config['allowed_types'] = 'jpg|jpeg|gif|png|pdf|xls|xlsx|csv|doc|docx|txt|zip|rar';
            $config['max_size'] = 10000;
            $config['file_name'] = $idForm . '_' . $ReviewNo . '_comment';
            $this->load->library('upload', $config);
            if (!$this->upload->do_upload('attach_files_comment')) {
                $file = '';
            } else {
                $data = array('upload_data' => $this->upload->data());
            ...
            $zip = new ZipArchive();
            $filename = $idForm . '_' . $ReviewNo . '_comment' . ".zip";
            $fileconpress = $download_folder . $filename;
            foreach ($_FILES['attach_files_comment']['name'] as $key => $files) {
                $nameFile = $files;
                $tmpName = $_FILES['attach_files_comment']['tmp_name'][$key];
                $conpress = $zip->open($fileconpress, ZIPARCHIVE::CREATE);
                if ($conpress === true) {
                    $zip->addFile($tmpName, $nameFile);
                    $zip->close();
...
//                    echo " Oh No! Error";
            ...
            $EditArr['attach_files_comment'] = $filename;
            $InserData = $Custom->Edit($EditArr, 'idApplicationComment', $idApplicationComment, 'application_comment');
            echo $InserData;
        }
    }
